...
| * The main "CONS" of WMS Reflector is that it is only used to preview.       |   * The main "CONS" of this is that the code is complex, not easy to understand and also take more time than WMS(Reflector)  |                     
| * So only a Single Layer could be shown at a time i.e you cannot show even the base layer.   |     * The code is nor Readable neither user friendly.          | 
